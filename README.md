...
Created two EKS clusters (one manual, one via GitHub Actions workflow) representing different NS environments.


Installed and configured Argo CD, including:


Admin role


“crunchops” viewer role
...
Set up Slack integration for Argo CD notifications to alert on:
...
Configured Argo CD Projects, Applications, RBAC policies, and end-to-end GitOps structure.


Implemented CloudWatch Container Insights dashboards with cross-account access (“monitor” vs “source” accounts).


Validated cluster metrics, pod health, logs, CPU/memory trends, and deployment behaviors.


Delivered the full demo to internal NS-facing ProServe team including Sameer, Puru, Raja, Nick, and Imad.


Documented all steps — EKS setup, GitOps repo structure, ArgoCD UI walk-through, CloudWatch dashboards, Slack integration — for internal reuse.
...
Provided a fully working NS-aligned GitOps + Observability blueprint that the internal team can extend into customer-ready patterns.


Improved team readiness by giving them a complete understanding of how EKS, Argo CD, CloudWatch, and Slack integrate in a real NS-like workflow.


Enabled Sameer, Raja, and Puru to validate and refine their proposed GitOps architecture for NS.


Significantly reduced onboarding time for new engineers supporting the NS engagement by providing a ready-to-use demo and documentation.


Strengthened NS internal solution direction by ensuring GitOps and observability follow AWS best practices.
